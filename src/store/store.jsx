...
// Add these actions to your store.js

// Force sync a single order
export const forceSyncOrder = (id) => async (dispatch) => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.post(
      `https://fanta-lib-back-production.up.railway.app/api/commandes/${id}/force-sync`,
      {},
      {
        headers: {
...
// Bulk sync statuses
export const bulkSyncStatuses = (parcelCodes) => async (dispatch) => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.post(
      'https://fanta-lib-back-production.up.railway.app/api/commandes/bulk-sync-statuses',
...
export const syncAllPending = () => async (dispatch) => {
  try {
    const token = localStorage.getItem('token');
    const response = await axios.post(
